...
>
> **Live:** `/p301` · **Code:** `src/p301-dashboard/` · **Commits:** prefixed `[P301]`

## Summary

A desktop dashboard for **Rosa, a first-time investor who runs her own starter account**. Once a week she sits down with it and, in about ten minutes, answers four questions without digging through anything:
...
**The call we made: attention first.** The brief asks: *"What would make their morning easier? What would tell them something's off before it becomes a problem?"* The account's owner is its operator, so the first thing on screen is not her balance but **what's off**: a returned deposit, money waiting in cash, a fee about to rise, a plan falling behind. Each item says what happened, what it means and what she can do about it, then lets her mark it handled.
...
|---|---|
| Account balances | Balance, money put in, up/down, cash waiting |
| Transaction volume | Activity: every deposit, buy and dividend, filterable |
| Risk flags | The attention list, and "ups and downs" ratings (1–5) on every fund |
...
Reads only from `src/shared/data/` (see BRIEF.md §4). Key facts, checked by the validator:
...
| Severity | Flag | Why it's there | What she can do (never "what to buy") |
|---|---|---|---|
...
| Heads-up | FL-GREEN is raising its yearly fee on Oct. 1 | Announced Sept. 15: 0.45% → 0.75% on $48.63 ≈ $0.15 more a year | **Open FL-GREEN** to see its fee over time |
| FYI | FL-BOND paid you $0.50 | Dividend on Sept. 15 | Nothing to do |

- Every flag can be **marked handled** (remembered for the session; there's an "Undo").
- Flags raised since last Sunday's review (Sept. 13) show a **New** badge.
...
**Desktop, 1280px and up:** a side rail (Weekly review · Your funds · Activity · Practice · Words to know) and a 12-column main area on cream.

```
┌ rail ┐┌───────────────────────────────────────────────────────────────────┐
│      ││ Good morning, Rosa.  Prices as of Fri., Sept. 18        [Demo ▾]    │
│ Week ││ ┌─ Needs your attention (7 col) ──────────┐┌─ Balance (5, PANEL) ──┐│
│ Funds││ │ ● Needs you  Deposit sent back       › ││ $1,313.72             ││
│ Activ││ │ ◐ Heads-up   Goal $150 behind plan   › ││ Up $63.72 on the      ││
│ Pract││ │ ◐ Heads-up   $154.76 waiting in cash › ││ $1,250 you put in     ││
│ Words││ │ ◐ Heads-up   FL-GREEN fee going up NEW›││ This week: up $15.57  ││
│      ││ │ ○ FYI        FL-BOND paid you $0.50 NEW││ Auto-invest: paused   ││
│      ││ └─────────────────────────────────────────┘└───────────────────────┘│
│      ││ ┌─ Money you put in vs. what it earned (12 col, PANEL) ── 1M 3M All ┐│
...
- **The attention list comes first** (top-left, where the eye lands). The balance sits beside it in the dark panel, the futuristic half of the brand.
- **Tablet (768–1023px):** the rail becomes top tabs; two columns; the chart goes full width.
- **Phone (under 768px):** one column in this order: attention, balance, chart, mix, goal, this week. Navigation moves into a menu. Fully usable, though not the design target.

## Interactions (the core flows a reviewer can test)

| # | Flow | Steps | Done when |
|---|---|---|---|
| F1 | **Act on a flag** | Click a flag → a side panel shows *what happened*, *what it means* (with term explanations) and *what you can do* → use its action → **Mark as handled** | Every flag opens, closes with Esc and has its action. Money actions open a demo dialog. Handled flags move to a collapsed "Handled" group with Undo. |
| F2 | **Understand any word** | Click or tab to any dotted-underlined term → the explanation opens → follow a "Related word" | Works by mouse, keyboard and screen reader on every page |
| F3 | **Read the growth chart** | Switch 1M / 3M / All → move across the chart → **Show as table** | Values are read out in words; the table matches the chart |
| F4 | **Look at a fund** | Your funds → FL-GREEN → price chart (since you bought / 1 year / 5 years), yearly fee with the Oct. 1 change, ups and downs 1–5, what's inside, what you paid vs. its value | Every fund page works, including FL-CALM, which Rosa doesn't own ("You don't own this fund") |
...
| F6 | **Practice with pretend money** | Practice → pick a fund → amount → review → confirm → see what you own in Practice and your practice mix → **sell** part of it → **Time machine**: how this mix would have moved over 5 made-up years → **Start over** | The "Practice money, not real" banner is always visible; the real account never changes |
| F7 | **Look up a word** | Words to know → search "fee" → open "Yearly fee" | A search with no results shows a helpful empty state |
| F8 | **Switch demo scenario** | Demo ▾ → *Nothing needs you* → *Brand-new account* | Each scenario shows its own account, and every sentence on every card stays true |
...
| **Down, not up** | FL-GREEN is down $6.37. Losses show the word "down", a minus sign and terracotta, plus an explanation that ups and downs are normal. |
| **All flags handled** | "You've handled everything for this week." Undo stays available. |
...
| # | Done when… | LI |
|---|---|---|
| 1 | `/p301` loads on the live site, directly and from the landing page | 1 |
| 2 | Flows F1–F8 work end to end on desktop, each with a passing Playwright test | 2 |
| 3 | The build matches this brief: Rosa, retail investing, attention-first, every term explained | 3, 6 |
| 4 | Nothing-needs-you, brand-new, all-handled, every Practice error and every empty filter or search are handled | 7 |
| 5 | Usable at 390px, 768px and 1280px; no horizontal scroll except inside tables | 8 |
| 6 | Repo root has README (with a P301 reviewer block) and LICENSE; `.claude/`, `CLAUDE.md`, `STATUS.md` and `docs/` hold the AI scaffolding; P301 code lives only in `src/p301-dashboard/` | 9, 10, 11 |
...
| 11 | The attention-first call is visible in the build: the list sits top-left, above the balance in reading order | 26, 27 |
