...
- Buys use that day's closing price and allow parts of a share (4 decimal places for stocks, 8 for crypto). **Stock buys settle one business day later (T+1)**, matching U.S. rules since May 28, 2024; crypto buys settle the same day.
...
4. **No disclaimer on screen** (ruling B, Phase 2.5). The only data notes are the CoinGecko credit next to crypto prices and the stock data note on stock charts and stock pages (§1). The README explains the project for reviewers.
