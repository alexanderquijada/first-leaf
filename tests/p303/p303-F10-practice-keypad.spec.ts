...
test('the keypad types the amount; each error shows above it and Review stays disabled', async ({ page }) => {
  ...
  await clear(page)
  await keys(page, '10')
  await expect(err).toHaveText('You do not own any FL-BROAD in Practice.')
  await expect(review).toBeDisabled()
})

test('review in a bottom sheet, confirm, and the banner is always visible', async ({ page }) => {
  await page.goto('/practice')
  await expect(page.getByRole('note').filter({ hasText: "Practice money. It isn't real money." })).toBeInViewport()
  await page.getByRole('radio', { name: /FL-BOND/ }).check()
  await keys(page, '200')
  await expect(page.locator('.porder__amount')).toHaveText('$200')
  await page.getByRole('button', { name: 'Review' }).click()
  const sheet = page.getByRole('dialog', { name: 'Check your order' })
  await expect(sheet).toContainText('Buy $200.00 of FL-BOND')
  await sheet.getByRole('button', { name: 'Confirm' }).click()
  await expect(page.getByText('You bought $200.00 of FL-BOND with practice money.')).toBeVisible()
