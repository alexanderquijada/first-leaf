...
test('every order error shows inline and keeps Review disabled', async ({ page }) => {
  ...
  const cases: [string, 'Buy' | 'Sell', string][] = [
    ...
    ['10', 'Sell', 'You do not own any FL-WORLD in Practice.'],
  ]
  for (const [amount, side, message] of cases) {
    await order(page, side, 'FL-WORLD', amount)
...
test('buy, see it, sell part, sell too much, time machine, start over', async ({ page }) => {
  await page.goto('/practice')
  await order(page, 'Buy', 'FL-BROAD', '200')
  await page.getByRole('button', { name: 'Review' }).click()
  await expect(page.getByText(`Buy $200.00 of FL-BROAD at ${money(price('FL-BROAD'))} a share.`)).toBeVisible()
  await page.getByRole('button', { name: 'Confirm' }).click()
  await expect(page.getByRole('status').filter({ hasText: 'You bought' })).toHaveText(/You bought \$200.00 of FL-BROAD with practice money./)
  await expect(page.locator('.practice__sum')).toContainText('$800.00')
  const row = page.locator('.practice__table tbody tr')
  await expect(row).toHaveCount(1)
  const shares = Math.floor((200 / price('FL-BROAD')) * 10000 + 1e-9) / 10000
  await expect(row).toContainText(shares.toFixed(4))

  // Sell more than she owns, then part of it.
  await page.getByRole('button', { name: 'New order' }).click()
  await order(page, 'Sell', 'FL-BROAD', '500')
  await expect(page.locator('#practice-error')).toContainText('of FL-BROAD in Practice. Enter that much or less.')
  await expect(page.getByRole('button', { name: 'Review' })).toBeDisabled()
  await page.getByLabel('Amount in dollars').fill('50')
  await page.getByRole('button', { name: 'Review' }).click()
  await page.getByRole('button', { name: 'Confirm' }).click()
  await expect(page.getByText('You sold $50.00 of FL-BROAD.')).toBeVisible()
  ...
  expect(series.length).toBe(funds[0].history.weekly.length)
  ...
  await expect(page.getByText('Nothing yet. Buy a fund to start.')).toBeVisible()
...
test('the banner stays on screen while scrolling', async ({ page }) => {
  ...
  const box = (await page.getByRole('note').filter({ hasText: "Practice money. It isn't real money." }).boundingBox())!
...
async function readAccount(page: Page) {
  ...
  await go(page, 'Funds', 'Your funds')
...
for (const scenario of ['normal', 'all-clear', 'brand-new']) {
  test(`a full practice session never changes the account (${scenario})`, async ({ page }) => {
    ...
    await order(page, 'Buy', 'FL-GREEN', '400')
    await page.getByRole('button', { name: 'Review' }).click()
    await page.getByRole('button', { name: 'Confirm' }).click()
    await page.getByRole('button', { name: 'New order' }).click()
    await order(page, 'Buy', 'FL-CALM', '300')
    await page.getByRole('button', { name: 'Review' }).click()
    await page.getByRole('button', { name: 'Confirm' }).click()
    await page.getByRole('button', { name: 'New order' }).click()
    await order(page, 'Sell', 'FL-GREEN', '100')
    await page.getByRole('button', { name: 'Review' }).click()
    await page.getByRole('button', { name: 'Confirm' }).click()
