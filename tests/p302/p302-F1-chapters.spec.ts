...
test('chapter 2: tapping a part reads it out', async ({ page }) => {
  ...
  const a = load('account'), inPct = Math.round((a.moneyIn / a.balance) * 100)
  await expect(ch2.locator('.share__readout')).toHaveText(`${money(a.gainLoss)} is what it earned. That is ${100 - inPct}%.`)
  await ch2.getByRole('button', { name: 'Money you put in' }).click()
  await expect(ch2.locator('.share__readout')).toHaveText(`${money(a.moneyIn)} of your balance is money you put in. That is ${inPct}%.`)
})

test('a chapter link lands on its chapter', async ({ page }) => {
  await page.goto('/story#chapter-3')
  const month = load('story-p302').rosaStory['rosa-starter'].facts.dip.month
  await expect(page.getByRole('heading', { name: `The dip in ${month}` })).toBeInViewport()
