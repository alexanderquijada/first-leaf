...
All data is fictional and generated by `scripts/generate-data.mjs`. `npm run validate` checks **47 rules**: the math adds up, the same numbers agree across every lens and every scenario, every sentence Your money story says about Rosa is recomputed from her history (R1–R4), no real tickers or brands appear, there's no advice language and no project language on screen (G6), and every learner-facing text reads at grade 8 or below. `npm run validate:selftest` proves each rule catches a deliberately broken dataset (83 cases, including every real defect found so far). A pre-commit hook runs every check before each commit, and a Playwright crawl confirms no project language appears anywhere on screen.
